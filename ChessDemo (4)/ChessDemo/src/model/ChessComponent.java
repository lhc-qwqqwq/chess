...
//    private static final Dimension CHESSGRID_SIZE = new Dimension(1080 / 4 * 3 / 8, 1080 / 4 * 3 / 8);
    ...
    private static final Color[] BACKGROUND_COLORS = {Color.WHITE, new Color(136,98,7)};
    ...
    public int name= 0;
    public int sta;
    public int count=0;//该棋子走了几步
    public boolean GuoLuBing=false;
    public boolean choosed=false;//是不是被划过的棋子
    public boolean canBeMoveTo=false;
    public static ChessComponent canGuoLuBing=null;
    ...
    protected void processMouseEvent(MouseEvent e) {
        ...
        if(e.getID()==MouseEvent.MOUSE_ENTERED){//鼠标进入
            System.out.printf("Move in [%d,%d]\n", chessboardPoint.getX(), chessboardPoint.getY());
            clickController.mouseMovein(this);
        }
        if(e.getID()==MouseEvent.MOUSE_EXITED){//鼠标出去
            System.out.printf("Move out [%d,%d]\n", chessboardPoint.getX(), chessboardPoint.getY());
            clickController.mouseMoveout(this);
        }
    }
    ...
    //public Color squareColor ;//= BACKGROUND_COLORS[(chessboardPoint.getX() + chessboardPoint.getY()) % 2];
   ...
   // }
    ...
    protected void paintComponent(Graphics g) {
        ...
        Color squareColor;
        if(choosed==false&&canBeMoveTo==false) squareColor = BACKGROUND_COLORS[(chessboardPoint.getX() + chessboardPoint.getY()) % 2];
        else if(choosed==true)squareColor=new Color(0,255,0);//如果鼠标划过，令其背景为红色
        else squareColor=new Color(0,0,255);
